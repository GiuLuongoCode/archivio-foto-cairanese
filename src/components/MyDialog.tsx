...
export default function MyDialog({
  ...
  const [imageDimensions, setImageDimensions] = useState({});
    ...
    const SEARCH_IMAGES_FILTERED = gql`
        query SearchImages($search: String, $limit: Int, $offset: Int) {
        searchImages(query: $search, limit: $limit, offset: $offset) {
            count,
            images {
                id
                name
    ...
    const { loading, error, data } = useQuery(SEARCH_IMAGES_FILTERED, {
      variables: { query: searchKey, limit: 4, offset: 0 }
    });
                ...
                >
                  <DialogPanel className="w-full relative max-w-2xl mx-auto lg:max-w-5xl xl:max-w-7xl rounded-xl flex flex-col bg-[#fff] justify-center items-center p-6 backdrop-blur-xl mt-10">
                    ...
                    <div className="relative flex items-center">
                      <button>
                        <svg
                          ...
                          viewBox="0 0 24 24"
                          strokeWidth={2.3}
                          stroke={`${idx > 0 ? "#e5e5e5" : "#e5e5e5"}`}
                          className={`
                          fixed top-10 md:top-[24rem] left-0 md:-left-20
                          ${
                            idx > 0
                          ...
                          <div className="w-2/3 flex flex-col">
                            ...
                            <div className="pt-8">
                              <div className="font-semibold">
                                ...
                                <div className="flex gap-2 flex-wrap text-base font-medium">
                                  {imageData.people.map((person: any, index: number) => (
                                    <>
                                    <span
                                      key={index}
                            ...
                            <div className="pt-8">
                              <div className="flex flex-wrap gap-2">
                                {imageData.tags.map((tag: any, index: number) => (
                                  <button
                                    key={index}
                      ...
                      <button>
                        <svg
                          ...
                          viewBox="0 0 24 24"
                          strokeWidth={2.3}
                          stroke={`${idx < limit - 1 ? "#e5e5e5" : "#e5e5e5"}`}
                          className={`
                          fixed top-10 md:top-[24rem] right-0 md:-right-20 size-8 ml-4 z-50 hover:cursor-pointer`}
                        >
                          <path
                            strokeLinecap="round"
                            strokeLinejoin="round"
                    ...
                    <div className="w-full py-24">
                      <div className="pb-4">
                        <h3 className="text-black text-lg font-semibold">Foto collegate:</h3>
                      </div>

                      {
                        error && <div className="max-full grid grid-cols-2 gap-2 justify-center ">
                          <h2>Fetch error</h2>
                        </div>
                      }
                      
                      { loading && <div className="max-w-4xl mx-auto grid grid-cols-2 gap-2 justify-center ">
                          <div className="w-full h-80 bg-[#bcbcbc] animate-pulse"></div>
                          <div className="w-full h-80 bg-[#bcbcbc] animate-pulse"></div>
                          <div className="w-full h-80 bg-[#bcbcbc] animate-pulse"></div>
                          <div className="w-full h-80 bg-[#bcbcbc] animate-pulse"></div>
                        </div>
